...
for epochs,step_size,gamma,weight_decay,p,lr,batch_size in product(*param_values):
    ...
    net = model()#, init_weights=Truev

    model_weight_path = "./weight/tmp.pt"   #导入之前训练好的权重
    ...
    # net.classifier.weight.requires_grad = True  #not sequential
    # net.classifier.bias.requires_grad = True

    # for layer in net.classifier.modules():
    ...
    save_path = './weight_transfer/best.pth'
    best_acc = 0.0
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if epoch==(epochs-1):
                ...
                np.save('X.npy', XX)
                np.save('Y.npy', YY)
                os.rename('X.npy', f'z_trans_X_{epoch}.npy')
                os.rename('Y.npy', f'z_trans_Y_{epoch}.npy')
...
# #------------------------------------------------------------------------------------------------------------------------
    ...
    net.load_state_dict(torch.load('./weight/tmp.pt'))   #默认应该本次训练的net
...
#                  os.cpu_count()                   Python中的方法用于获取系统中的CPU数量
    ...
    # 保存文件
    workbook.save('./result_files/trans_acc_time.xls')